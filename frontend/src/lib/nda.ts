 ...
 *
 * Most clauses are static boilerplate. Clause 9 (Governing Law and
 * Jurisdiction) is interpolated with the user's values so the executed
 * document reads as complete.
 */
export function getStandardTerms(data: NdaData): Clause[] {
  const law = valueOr(data.governingLaw, "[State]");
  const jurisdiction = valueOr(
    data.jurisdiction,
 ...
 * when neither party has a company name.
 */
